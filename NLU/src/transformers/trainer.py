            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.deepspeed:
                        ...
                        # step >= self.rankallocator.total_step-self.rankallocator.final_warmup
                        # for n, p in model.named_parameters():
                        #     if n == "deberta.encoder.layer.0.attention.output.dense.lora_E":
                        #         weight = float(p[0])
                        #         grad = float(p.grad[0])
                        #         step = global_step
                        #         with open("/data2/wjh/AdaLora/NLU/examples/text-classification/output/score_variation/MNLI.txt", "a") as f:
                        #             f.write(f"step:{step}   weight:{weight} grad:{grad}\n")

                    if not self.deepspeed:
                        self.lr_scheduler.step()

                    if isinstance(self.rankallocator, RankAllocator):
                        model.zero_grad() # 在进行初始AdaloRA的时候，因为梯度先设置为0了，所以计算不出重要性。

                    if self.rankallocator is not None:
                        # Apply AdaLoRA to allocate the budget 
                        curr_rank, mask_threshold = self.rankallocator.update_and_mask(self.model, self.state.global_step, self.args.root_output_dir) # 计算重要性并mask ， 只有这边是AdaLora多花的时间

                    if isinstance(self.rankallocator, RankAllocator_init):
                        model.zero_grad() # 在进行初始AdaloRA的时候，因为梯度先设置为0了，所以计算不出重要性。
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.model_args and self.model_args.apply_lora and self.model_args.reg_orth_coef>0:
            ...
            if self.model_args.lora_type=="frd": 
                ...
                regu_loss = self.compute_svd_orth_regu(model)    # 正交损失
                if isinstance(self.rankallocator, RankAllocator):
                    if self.rankallocator.global_step+1 > self.rankallocator.initial_warmup and self.rankallocator.global_step+1<=self.rankallocator.total_step - self.rankallocator.final_warmup:
                        self.rankallocator.regu_loss.append(regu_loss)  # 这边要进行判断，是否是原始的AdaLoRA。
                # with open(f"{self.args.root_output_dir}/regu_loss.txt","a") as f:
                #     f.write(f"global_step: {self.rankallocator.global_step}, regu_loss: {regu_loss}")
            else:
